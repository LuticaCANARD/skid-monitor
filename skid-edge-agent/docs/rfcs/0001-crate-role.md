...
현재 구현은 실제 센서 대신 deterministic mock sample을 보낸다.
...
hardware-specific code는 future sensor layer로 분리하고, protocol 전송 경계는 작게 유지한다.

## Non-Goals

- Kubernetes node agent가 아니다.
- agent device socket을 listen하지 않는다.
- file, compute, media payload를 만들지 않는다.
