...
| Status | Draft |
| Created | 2026-06-27 |
| File | `client/skid-monitor-client/docs/rfcs/0002-csharp-extension-developer-experience.md` |
| Scope | `skid-monitor-client`, `client/skid-monitor-client/bindings/dotnet` |
...
- Unity 6 managed plug-in 호환을 위해 SDK와 extension assembly는 `netstandard2.1`을 target으로 둔다.
- C# extension은 raw signal JSON 문자열을 받아 필요한 signal만 해석하고, 무거운 처리는 비동기로 넘긴다.
...
현재 `SkidSignalContext.SignalJson`은 raw JSON 문자열이다. SDK public API가 `System.Text.Json` 같은
특정 runtime library 타입에 묶이지 않도록 의도적으로 문자열 경계를 유지한다. C# 개발자는 Unity
환경에서 사용하는 JSON parser로 필요한 field만 점진적으로 읽는 방식이 안전하다. 예외가 extension
host 전체를 불안정하게 만들지 않도록, parsing 실패는 extension 내에서 처리한다.
