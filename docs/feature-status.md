...
2026-07-19에 native FE + agent smoke를 확인했고, 2026-07-21에 VRM animation/MToon 구현을
포함한 자동 검증을 다시 실행했다.

- `cargo test --workspace`: 175 passed, 0 failed, PostgreSQL integration test 1개 ignored
- `cargo test -p skid-monitor-fe --lib --no-default-features --features high-spec`: 106 passed, 0 failed
