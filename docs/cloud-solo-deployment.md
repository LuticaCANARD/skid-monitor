...
| 인증/ACL | 없음. 숫자형 loopback 주소만 허용 | Keycloak JWT + tenant RLS |
...
두 서버는 서로 다른 공개 포트와 Keycloak audience를 사용한다.
...
- 두 token 모두 UUID 문자열인 `tenant_id` claim을 가져야 한다.
- ingress의 agent ID는 token의 `azp` 또는 `client_id`에서 가져온다. 요청 body가 agent/tenant를
  자칭할 수 없다.
...
두 resource client의 `resource_access.<audience>.roles`에 해당 역할이 나타나도록 mapper를 설정하고,
사용자/agent의 조직 UUID를 `tenant_id` access-token claim으로 매핑한다. realm role이나 다른 audience의
동명 role은 인정되지 않는다. issuer, discovery 문서의 issuer, JWKS URL은 모두 같은 HTTPS realm URL
아래에 있어야 한다. access token은 `iat`를 포함해야 하며 수명은 최대 24시간으로 제한한다.
realm/client의 access-token 서명 알고리즘은 현재 verifier가 허용하는 `RS256`으로 설정한다.
...
JWT에서 검증된 UUID를 `app.tenant_id` local setting으로 먼저 넣는다. 운영 DB 계정에는
runtime은 직접 login으로 연결하고 `BYPASSRLS`, superuser, `CREATEDB`, `CREATEROLE`, `REPLICATION`이나
...
export SKID_MONITOR_KEYCLOAK_ISSUER='https://id.example.com/realms/skid-monitor'
export SKID_MONITOR_TENANT_CLAIM='tenant_id'
```
...
```json
{
  "exporters": {
    "cloud": {
      ...
      "auth": {
        "token_url": "https://id.example.com/realms/skid-monitor/protocol/openid-connect/token",
        "client_id": "agent-node-a",
        "client_secret_env": "SKID_MONITOR_AGENT_CLIENT_SECRET",
...
원래 Keycloak token의 `exp`도 보존하며, 연결된 WebSocket은 그 시각에 종료되어 재인증을 요구한다.
...
client-access server는 same-origin reverse proxy 경로 아래에 둔다. Keycloak Authorization Code + PKCE
login/refresh를 소유한 host shell은 access token을 `sessionStorage`의
`skid-monitor.keycloak.access_token` key에 넣는다. adapter는 연결 시마다
...
spool에 영속화하지는 않는다. state volume을 삭제/rollback하거나 같은 Keycloak client ID를 서로 다른
