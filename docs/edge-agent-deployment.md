...
이 문서는 `skid-edge-agent`와 `skid-monitor-agent`의 관계, 배포 경계, 설치 방식의
기본 결정을 명시한다.

## Role

`skid-edge-agent`는 현장 장비 가까이에서 물리 계층과 환경 신호를 만드는 probe다.
`skid-monitor-agent`는 중앙 또는 사이트별 수집 지점으로, edge agent가 보낸 `Signal`을 받아
...
따라서 `skid-edge-agent`는 `skid-monitor-agent` 안에 포함되는 하위 모듈이 아니다. 같은
workspace와 `skid-protocol` 계약을 공유하지만, 실행과 배포 단위는 별도 바이너리로 둔다.
...
```text
   Edge nodes / gateways               skid-monitor-agent (tokio)            client
 +--------------------------+      +------------------------------+
 ...
 +--------------------------+  /   |        ^                     |          (화면)
 | skid-edge-agent #N    |=/    |  self OTel + system_metrics  |
 +--------------------------+      +------------------------------+
...
| 값 | 의미 | 운영 판단 |
| --- | --- | --- |
| `SKID_MONITOR_DEVICE_LISTEN_ADDR=off` | device ingress 비활성화 | edge agent를 받지 않는 server |
...
하나의 `skid-monitor-agent`에는 여러 `skid-edge-agent`가 연결될 수 있다.
...
sequenceDiagram
    ...
    loop 매 SKID_MONITOR_EDGE_INTERVAL_SECS 주기
        A->>S: TCP connect
...
- **agent 송신부**(`skid-edge-agent/src/main.rs`)는 동기 `std::net::TcpStream`을 쓴다.
...
edge agent의 다음 hop은 `skid-monitor-agent` → `skid-monitor-client` 구간이다. edge agent 배포 자체는
...
2. 설치 스크립트가 바이너리를 `/usr/local/bin/skid-edge-agent`에 배치한다.
3. 설정 파일을 `/etc/skid-monitor/edge-agent.env`에 만든다.
4. systemd unit을 `/etc/systemd/system/skid-edge-agent.service`에 만든다.
5. `systemctl enable --now skid-edge-agent`로 부팅 시 자동 실행되게 한다.
...
agent와 server가 공유하는 계약은 `skid-protocol` 크레이트 한 곳에 모여 있다. 실제 코드 기준으로
...
```text
Signal (enum)
 ├─ Metrics(ExportMetricsServiceRequest)    <── edge-agent가 보내는 유일한 variant
 ├─ Traces(ExportTraceServiceRequest)
 └─ Logs(ExportLogsServiceRequest)
...
- **계약 표면 vs. 사용 범위**: `Signal`은 `Metrics`/`Traces`/`Logs` 3 variant를
  가지지만, edge agent는 `Signal::Metrics`만 생성한다. 반대로 server 수신부는 3종을 모두
...
`skid-edge-agent`와 `skid-monitor-agent`는 독립 실행 파일이지만 같은 `skid-protocol` 계약을
...
workspace는 `skid-protocol`(공유 계약), `skid-edge-agent`, `skid-monitor-agent`,
`skid-monitor-client` 네 크레이트로 구성된다(edition 2024, resolver 3). 의존 방향은 다음과 같다.
...
```text
    ...
    +---------+          |          +----------+
    |                    |                     |
 skid-edge-agent  skid-monitor-agent     skid-monitor-client
 skid-protocol      skid-protocol +    skid-protocol +
 + serde_json       tokio + OTel +     serde_json
 std::net/thread    serde_json + libc  + extension bridge
 (런타임 없음)      (Linux 묶임)
```
...
- **`skid-protocol`는 의도적으로 런타임을 갖지 않는다**: `serde`와 OTLP proto 타입은 들고
  ...
  무겁게 만들지 않으려는 결정이며, agent가 server의 무거운 스택을 상속하지 않게 하는 핵심 장치다.
- **edge-agent는 최소 의존**: `skid-protocol + serde_json`만 쓴다. 비동기 런타임도, OTel도,
...
`skid-monitor-agent`는 edge 신호를 받아 client로 전달하는 gateway이고, `skid-edge-agent`는
