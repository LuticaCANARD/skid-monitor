...
| Scope | `skid-protocol`, `skid-monitor-agent`, `skid-monitor-client`, `skid-edge-agent`, `skid-file-node`, `skid-compute-advisor`, future edge media provider mode |
...
이 RFC는 file transfer, SKDM v1, media, production enrollment까지 함께 다루는 umbrella RFC이므로
전체 status는 `Draft`다. 아래 checklist는 2026-07-19 code 상태를 기록할 뿐, unchecked 결정을
채택됐거나 구현 기준이라고 선언하지 않는다. 제품 기능 상태는
[Feature Status](../feature-status.md)가 정준 출처다.

- [x] Shared OTLP `Signal` contract와 legacy length-prefixed JSON framing
- [x] Agent receiver/processor/exporter pipeline
- [x] OTLP metrics/logs/traces receiver
- [x] Device ingress와 compact `skid-edge-wire` adapter
- [x] Linux와 macOS host sampler
- [x] Database log file receiver
- [x] PostgreSQL/OIDC split cloud ingress와 client access
- [x] Native multi-listener control-room frontend
- [ ] SKDM v1 versioned/authenticated framing
- [ ] Production device enrollment, authentication, rate/connection limits
- [ ] Actual GPIO/I2C/serial/MCU sensor adapter
- [ ] Authorized file offer/transfer plane
- [ ] Production package, upgrade, rollback artifact
- [ ] Durable agent payload spool와 crash-safe replay

## Detailed Introduction

Skid Monitor는 애플리케이션, Linux host, edge 장비, read-only file node, compute capability,
edge media provider에서 나오는 신호를 하나의 가벼운 관측 흐름으로 모으려는 실험이다. 첫 단계의
목표는 "무엇을 실행할 수 있는가"보다 "어디에서 무엇을 관측했고, 그 신호를 어떤 경계로 안전하게
흘릴 것인가"를 고정하는 것이다. 그래서 이 RFC는 기능 목록이 아니라 초기 통합 경계에 대한 문서다.

현재 workspace의 전체 package와 runtime 관계는 [Architecture](../architecture.md)를 따른다. 이 RFC
초안이 초기 통합 경계로 직접 다루는 여섯 역할은 다음과 같다.
...
- `skid-monitor-client`: frontend가 공유하는 signal receiver library와 C# extension host 경계
- `skid-edge-agent`: edge 물리/환경 신호를 만들고 future camera/media provider metadata를 관측하는 작은 probe
...
여기에 future quantum adapter와, edge-agent로 충분하지 않은 전문 media node가 붙을 수 있다. 단, future
node도 같은 원칙을 따른다. raw media frame, compute input 같은 큰 payload나 권한이 필요한 데이터는 초기 device socket으로
...
이 RFC가 제안하는 목표 topology는 다음과 같다. **현재 runtime topology가 아니며**, 구현 여부는
[Feature Status](../feature-status.md)에서 확인한다. 이 RFC에서 "제공자 서버"는 monitoring signal을
받아 사용자에게 제공하는 `skid-monitor-agent` 쪽 gateway를 뜻한다.
...
| | emits: edge.*, stream.*    | |      | | receives: device ingress    | |      | | renders: console summary    | |
...
flowchart LR
    ...
    HostMetrics --> Agent
    Edge -->|edge + stream Signal::Metrics| DeviceIngress
...
+------------------------------+                 +-------------------------------+       +----------------------------+
| skid-edge-agent              |-- Signal ------>| skid-monitor-agent            |--event>| skid-monitor-client        |
| emits: edge.* + stream.*     |                 | part: device ingress          |       | console / extension        |
| metadata only                |                 | part: Signal forwarder        |       +----------------------------+
+------------------------------+                 |                               |
                                                 |                               |
+------------------------------+                 |                               |
...
flowchart LR
    ...
    EdgeCap -->|edge / stream Signal::Metrics| DeviceIngress
...
- 기본 배포는 작은 Rust 바이너리와 OS native service manager다. Linux는 systemd, macOS는 launchd,
  Windows는 Windows Service를 기준으로 하며, Kubernetes는 Linux sidecar/cluster-internal 관측부터
  허용한다.
...
- edge physical signal, edge media provider metadata, file capability, compute capability를 같은 관측
  흐름에 태운다.
...
- `.deb`, `.rpm`, `.pkg`, `.msi`, Helm chart, Kustomize manifest 같은 실제 배포 산출물 포맷을
  확정하지 않는다.
...
정준 enum 변형 순서는 현재 구현 기준으로 `OpenTelemetry, Kubernetes, EdgeDevice, FileNode,
ComputeAdvisor, Quantum, System, MacOS`이다. `Stream`과 Windows source는 future 값이다.
...
| `Source::Stream` | `stream` | `edge_device` media provider role / optional `stream_node` | `skid-edge-agent` | future |
| `Source::Quantum` | `quantum` | node kind 미정 | 전용 adapter crate | future |
| `Source::MacOS` | `macos` | macOS native host collector | `skid-monitor-agent` | current |
| future Windows source | `windows` | Windows native host collector | `skid-monitor-agent` | planned |

핵심 비대칭은 두 가지다. `stream`은 source 식별자이고, 기본 producer node kind는 `edge_device`다.
별도 `stream_node`는 edge-agent로 충분하지 않은 전문 media relay/packager topology의 future option이다.
또한 `collector` node kind에는 단일 source가 대응하지 않고 `system`, `opentelemetry`, `kubernetes`
source를 함께 낼 수 있다.
...
| `stream_observer` | `edge_device` | stream health/preview metadata 관측 |
...
| `stream.*` | future `skid-edge-agent` media provider telemetry | `stream.source.up`, `stream.video.fps`, `stream.packets.lost` |
...
future media provider handling은 기본적으로 `skid-edge-agent`가 camera gateway 근처에서 stream
stats/status만 관측한다. edge-agent로 충분하지 않은 전문 relay/packager topology는 별도 stream node를
후속 설계로 둘 수 있다.
...
future skid-edge-agent media provider mode -> stream telemetry and preview metadata
...
기본 배포 방식은 단일 Rust 바이너리와 OS native service manager다. Linux host는 systemd,
macOS host는 launchd `LaunchDaemon`, Windows host는 Windows Service를 기준으로 한다.
edge/file/compute node는 장비 또는 gateway에서 계속 떠 있어야 하는 장기 실행 프로세스이며, 작은 단일
바이너리는 현장 장비에 설치, 교체, 롤백하기 쉽다. Linux systemd unit은 root 상시 실행 대신 전용
사용자와 필요한 device만 허용하는 권한 모델을 기본 형태로 둔다. macOS와 Windows package는 각 OS의
signing, service registration, uninstall/rollback 규칙을 따른다.
...
`skid-edge-agent` media provider mode와 `Source::Stream`은 future다. stream 기능은 media transport가
아니라 stream telemetry와 preview coordination plane이다. device socket에는 stream metadata와 health
metric만 보내고, raw video/audio frame은 보내지 않는다. camera/image/video provider를 edge-agent
확장 지점으로 받는 세부 계약은
[RFC 0002: Extensible Edge Media Provider Contract](0002-extensible-media-provider.md)를 따른다.
...
| node kind / role | `edge_device` / `stream_observer` |
| binding kind | `stream` |
| resource_type | `webrtc` |
| 바이너리 | `skid-edge-agent` |
...
Kubernetes 운용은 Linux agent 배포의 한 형태로만 조건부 가능하다. 현재 구현만 기준으로는 cluster
내부 관측, sidecar, DaemonSet 실험에는 적합하지만 공개 device ingress나 remote execution scheduler로
바로 쓰기에는 부족하다. macOS와 Windows native agent는 이 절이 아니라 OS package/service manager
배포 정책을 따른다.
...
release target은 Kubernetes image와 native OS package를 분리한다.

| 영역 | 우선 target | 산출물 |
| --- | --- | --- |
| Linux Kubernetes | `x86_64-unknown-linux-musl`, `aarch64-unknown-linux-gnu/musl` | OCI image |
| Linux native | `x86_64-unknown-linux-gnu`, `aarch64-unknown-linux-gnu`, `armv7-unknown-linux-gnueabihf` | `.deb`, `.rpm`, `tar.gz`, systemd unit |
| macOS native | `aarch64-apple-darwin`, `x86_64-apple-darwin` | signed/notarized `.pkg`, optional universal binary |
| Windows native | `x86_64-pc-windows-msvc`, later `aarch64-pc-windows-msvc` | signed `.msi`, Windows Service |

`install.sh`, systemd unit, launchd plist, Windows Service installer, env 템플릿, release CI,
`.deb`, `.rpm`, `.pkg`, `.msi`는 아직 산출물이 없다. 설치 UX는 목표 상태로만 기록한다. 운영 경로에는
체크섬/서명 검증, 멱등 재설치, 언인스톨, 롤백이 필요하다.
...
| Binary | Check items |
| --- | --- |
| `skid-monitor-agent` | client addr, device ingress listen addr, public bind guard, frame mode, OS sampler dependency, service manager 권한 |
...
19. `Source::Stream`, `skid-edge-agent` media provider mock stats, client stream summary를 future 단계로 추가한다.
...
- Edge media provider implementation: `skid-edge-agent` media provider adapter, source `stream`,
  RFC 0002 edge media provider contract.
